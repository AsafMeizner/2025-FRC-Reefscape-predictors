...
#########################################
# Part 2: Calculation-Based Predictor (unchanged)
#########################################
...
def calculate_estimated_alliance_score(data, team_numbers):
    ...
    # RP Conditions (max RP = 6)
    rp = 0
    # Coral Bonus: if each level (L1-L4) averages at least 5 corals across teams.
    levels = ["L1", "L2", "L3", "L4"]
    for level in levels:
        vals = []
        for t in team_numbers:
            ...
            if teamData:
                avg = sum(entry.get(level + "sc",0) + entry.get("t"+level+"sc",0) for entry in teamData) / len(teamData)
                vals.append(avg)
        if vals and (sum(vals)/len(vals)) >= 5:
            rp += 1
    # Autonomous Bonus: if every team leaves the starting line and scores a coral in auto.
    def avg_Mved(t):
        teamData = [entry for entry in data if entry["teamNumber"] == t]
        if not teamData:
            return 0
        return sum(1 if entry.get("Mved", False) else 0 for entry in teamData) / len(teamData)
    if all(avg_Mved(t) >= 0.5 for t in team_numbers):
        rp += 1
    # Barge Bonus: if the alliance’s average barge points exceed 14.
    def average_algae_barge(t):
        teamData = [entry for entry in data if entry["teamNumber"] == t]
        if not teamData:
            return 0
        return sum(entry.get("ScAb",0)*4 + entry.get("tScAb",0)*4 for entry in teamData) / len(teamData)
    avg_barge = sum(average_algae_barge(t) for t in team_numbers) / len(team_numbers)
    if avg_barge >= 14:
        rp += 1
    # Win Bonus: assume a fixed win bonus of 3 RP.
    rp += 3
    if rp > 6:
        rp = 6
...
#########################################
# Part 3: Enhanced Deep RL Environment & DQN Agent
#########################################
# We use the same environment rules but now we have more data (algae supply=36, full coral availability).
class ReefscapeEnv(gym.Env):
    def __init__(self, team_stats, focus='points'):
        self.team_stats = team_stats  # dictionary for 3 teams
        self.focus = focus  # 'points' or 'rp'
        self.total_time = 150.0  # seconds per match
        self.auto_time = 15.0
        self.teleop_time = self.total_time - self.auto_time
        self.time = 0.0
        self.phase = 'auto'
        self.num_teams = len(team_stats)
        self.coral_remaining = {'L1': 12, 'L2': 12, 'L3': 12, 'L4': 12}  # full availability
        self.algae_remaining = 36  # increased algae supply
        self.climb_status = {team: None for team in self.team_stats}
        self.alliance_points = 0.0
        self.alliance_rp = 0
        self.done = False

        # Each team has 13 possible actions:
        # 0: Do nothing
        # 1-4: Score coral on L1-L4 (auto/teleop scores differ)
        # 5: Score algae (barge)
        # 6: Score algae (processor)
        # 7,8: Block removal actions (no-op)
        # 9: Endgame: no climb, 10: park (2), 11: shallow (6), 12: deep (12)
    ...
    def _get_obs(self):
        climb_flags = [1 if self.climb_status[team] is not None else 0 for team in self.team_stats]
    ...
    def step(self, actions):
        ...
        if self.done:
            ...
            # RP conditions based on official rules:
            # 1. Coral threshold: at least 5 corals per level yields 1 RP.
            if all(coral_scored[lvl] >= 5 for lvl in coral_scored):
                self.alliance_rp += 1
            # 2. Autonomous bonus: if all robots leave the starting line and at least one coral is scored in auto.
            all_auto = all(self.team_stats[team].get('auto_left', 1.0) >= 0.5 for team in self.team_stats)
            if all_auto:
                self.alliance_rp += 1
            # 3. Barge bonus: if barge points exceed 14.
            barge_points = (36 - self.algae_remaining) * 4
            if barge_points >= 14:
                self.alliance_rp += 1
            # 4. Win bonus: fixed 3 RP (assumed when predicting win)
            self.alliance_rp += 3
            if self.alliance_rp > 6:
                self.alliance_rp = 6
...
# Dueling Double DQN Agent with joint action space (13^3 possible actions).
...
def train_agent(team_stats_pool, focus, agent, episodes=2000):
    for e in trange(episodes, desc=f"Training Episodes ({focus})"):
        ...
        if e % 20 == 0:
            agent.update_target()
            print(f"Episode {e}, Total Reward: {total_reward:.2f}, Points: {info['points']}, RP: {info['rp']}")
...
def main():
    ...
    # Aggregate team performance stats.
    ...
    alliance = [20, 11, 28]
    calc_score, calc_rp = calculate_estimated_alliance_score(data, alliance)
    print("\n=== Calculation-Based Alliance Prediction ===")
    print("Alliance Teams:", alliance)
    print("Estimated Total Points:", round(calc_score, 2))
    print("Estimated RP:", calc_rp)
    
    # --- Deep RL Prediction ---
    obs_size = 9  # [time, 4 coral levels, algae, 3 climb flags]
    num_joint_actions = NUM_ACTIONS_PER_TEAM ** 3  # 13^3 = 2197
    
    # Train agent for Points Focus.
    agent_points = DQNAgent(obs_size, num_joint_actions, lr=1e-3)
    print("\n=== Training DQN Agent for Points Focus ===")
    train_agent(team_stats_pool, 'points', agent_points, episodes=1000)
    ...
    # Train agent for RP Focus.
    agent_rp = DQNAgent(obs_size, num_joint_actions, lr=1e-3)
    print("\n=== Training DQN Agent for RP Focus ===")
    train_agent(team_stats_pool, 'rp', agent_rp, episodes=1000)
